...
for subdir, dirs, files in os.walk(srchstr):
    for file in files:
        ...
        if  filepath.endswith(".wav") and ("Drums" in str(filepath) and "Reggae" in str(filepath) and "Noise" not in str(filepath) and "OS" not in str(filepath) and "Blip" not in str(filepath) and "Euro" not in str(filepath)):
            contentbeats.append(filepath)

        if  filepath.endswith(".wav") and ("Reggae" in str(filepath)  and "Pad" in str(filepath)):
            contentdrones.append(filepath)

        if  filepath.endswith(".wav")  and ("Reggae" in str(filepath) and "Bass" in str(filepath)):
            contentbass.append(filepath)

        if  filepath.endswith(".wav") and ("Reggae" in str(filepath) and "Keys" in str(filepath)) :
            contentorg.append(filepath)

        if  filepath.endswith(".wav") and ("Reggae" in str(filepath) and "Brass" in str(filepath)) :
            contentsax.append(filepath)

        if  filepath.endswith(".wav") and ("Reggae" in str(filepath) and "Synth" in str(filepath)) :
            contentgit.append(filepath)

        if  filepath.endswith(".wav") and ("Reggae" in str(filepath)) and (("Pad" not in str(filepath)) and ("Drone" not in str(filepath)) and ("Beat" not in str(filepath)) and ("Drum" not in str(filepath))and ("Bass" not in str(filepath)) and ("Tone" not in str(filepath)) and ("OS" not in str(filepath)) and ("Signals" not in str(filepath))  and ("Dialog" not in str(filepath)) and ("Spoken" not in str(filepath)) and ("Guitar" not in str(filepath))) :
